...
const BASE_URL =
  "https://vod0qxda75.execute-api.ap-southeast-1.amazonaws.com/default/dengue-api";
const BASE_RISK_CDN_URL = "https://djez3n6us0xv.cloudfront.net/dengue-api";
...
const riskApi = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL ?? BASE_RISK_CDN_URL,
});
...
export async function getRiskMap(): Promise<PlanningAreaRisk[]> {
  const { data } = await riskApi.get<RiskApiResponse>("/risk");
